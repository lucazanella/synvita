...
#SBATCH --output=logs/entailment_inference_videocon_human_%A.out
...
input_csv="datasets/final_videocon_human.csv"
model_path="${TRANSFORMERS_CACHE}/models--LanguageBind--Video-LLaVA-7B/snapshots/aecae02b7dee5c249e096dcb0ce546eb6f811806"

process_ckpt() {
    batch_size=8
    if [ "$batch_size" -gt 1 ]; then
        local output_csv="$1/final_videocon_human_scores_batch.csv"
        local output_file="$1/videocon_human_results_batch.txt"
    else
        local output_csv="$1/final_videocon_human_scores_new.csv"
        local output_file="$1/videocon_human_results.csv"
    fi
    ...
    python src/eval_videocon_human.py \
        --data_dir "$data_dir" \
