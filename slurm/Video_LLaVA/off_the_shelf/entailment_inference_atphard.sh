...
process_ckpt() {
    ...
    if [ "$batch_size" -gt 1 ]; then
        local output_csv="$1/atphard_scores_batch.csv"
        local output_file="$1/atphard_results_batch.txt"
    else
        local output_csv="$1/atphard_scores_new.csv"
        local output_file="$1/atphard_results.txt"
    fi
