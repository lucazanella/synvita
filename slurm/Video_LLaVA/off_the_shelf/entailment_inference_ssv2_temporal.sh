...
#SBATCH --output=logs/entailment_inference_ssv2_temporal_%A.out
...
input_csv="datasets/eval_ssv2_temporal_updated.csv"
model_path="${TRANSFORMERS_CACHE}/models--LanguageBind--Video-LLaVA-7B/snapshots/aecae02b7dee5c249e096dcb0ce546eb6f811806"

process_ckpt() {
    batch_size=8
    if [ "$batch_size" -gt 1 ]; then
        local output_csv="$1/ssv2_temporal_scores_batch.csv"
        local output_file="$1/ssv2_temporal_results_batch.txt"
    else
        local output_csv="$1/ssv2_temporal_scores.csv"
        local output_file="$1/ssv2_temporal_results.txt"
    fi
