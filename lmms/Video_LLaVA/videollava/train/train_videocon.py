...
# from lmms.Video_LLaVA.videollava.train.llava_trainer_videocon import LLaVATrainer
from videollava.train.llava_trainer_videocon import LLaVATrainer
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        # collate_fn
        video = [instance["video"] if instance["video"] is not None else None for instance in instances]
        ...
        input_ids, labels = tuple([instance["text"][key] for instance in instances]
                                  for key in ("input_ids", "labels"))
        input_ids = torch.nn.utils.rnn.pad_sequence(
            input_ids,
            batch_first=True,
        ...
        videopaths = [instance["videopath"] for instance in instances]
        captions = [instance["caption"] for instance in instances]
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                data_args) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    from lmms.Video_LLaVA.videollava.train.data_utils.xgpt3_dataset import MultiModalDataset
    train_dataset = MultiModalDataset(
        ...
        data_args=data_args,
        max_length=tokenizer.model_max_length,  # TODO: check
