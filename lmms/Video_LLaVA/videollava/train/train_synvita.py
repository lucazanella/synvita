...
from lmms.Video_LLaVA.videollava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
...
from lmms.Video_LLaVA.videollava.train.llava_trainer_synvita import LLaVATrainer
# from videollava.train.llava_trainer_synvita import LLaVATrainer

from lmms.Video_LLaVA.videollava import conversation as conversation_lib
from lmms.Video_LLaVA.videollava.model import *
from lmms.Video_LLaVA.videollava.mm_utils import tokenizer_image_token
...
from lmms.Video_LLaVA.videollava.utils import order_pick_k
...
class DataArguments:
    ...
    num_frames: int = 8
    # ===================================================================
    features_folder: Optional[str] = field(default=None)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    sem_cons_weight: float = 1.0
    sem_cons_margin: float = 0.2
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        batch = [data for instance in instances for data in instance]
        # collate_fn
        ...
        # If all video data are pixel values (4D tensors)
        elif all([len(img.shape) == 4 for img in video if img is not None]):
            video = [img for img in video if img is not None]
        ...
        elif all([len(vid.shape) == 3 or len(vid.shape) == 4 for vid in video if vid is not None]):
            # Separate 4D (pixel values) and 3D (embeddings) tensors
            video_4d = [img for img in video if img is not None and len(img.shape) == 4]
            video_3d = [embed for embed in video if embed is not None and len(embed.shape) == 3]

            # if video_4d:
            #     video_4d = torch.cat(video_4d, dim=0)

            if video_3d:
                video_3d = torch.stack(video_3d, dim=0)

            video = {
                "video_pixel_values": video_4d,
                "video_embeds": video_3d
            }
        else:
            raise ValueError("Video size not supported")

        input_ids, labels = tuple([data["text"][key] for data in batch]
        ...
        shared_semantic_mask = [(data["text"]["shared_semantic_mask"]) for data in batch]
        shared_semantic_mask = torch.nn.utils.rnn.pad_sequence(
        ...
        shared_semantic_mask = shared_semantic_mask[:, :self.tokenizer.model_max_length]
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    from lmms.Video_LLaVA.videollava.train.data_utils.xgpt3_dataset_synvita import MultiModalDataset
    train_dataset = MultiModalDataset(
        video_folder=data_args.video_folder,
        features_folder=data_args.features_folder,
        input_file=data_args.data_path[0],
        tokenizer=tokenizer,
        processor=data_args.video_processor,
        data_args=data_args,
        max_length=tokenizer.model_max_length,
        use_extracted_features=data_args.use_extracted_features,
    )
