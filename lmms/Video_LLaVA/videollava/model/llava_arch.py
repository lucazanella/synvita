...
from lmms.Video_LLaVA.videollava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        self, input_ids, position_ids, attention_mask, past_key_values, labels, images, shared_semantic_mask, use_extracted_features=False, domains=None
    ):
        # ====================================================================================================
        ...
        '''
        # if not any(img.shape[-1] == self.get_model().mm_projector[-1].out_features for img in images if isinstance(img, torch.Tensor)):
        if use_extracted_features:
            ...
            if isinstance(images, dict):
                real_video_features_minibatch = images["video_embeds"]
                video_idx = [idx for idx in range(len(images["video_embeds"]) + len(images["video_pixel_values"]))]
                videos_minibatch = torch.stack(images["video_pixel_values"]) if len(images["video_pixel_values"]) > 0 else []  # mini_b c t h w
                assert len(video_idx) == len(images["video_embeds"]) + len(images["video_pixel_values"])
            else:
                real_video_features_minibatch = images
                video_idx = [idx for idx, vid in enumerate(images)]
                videos_minibatch = []
                assert len(video_idx) == len(images)
        else:
        ...
        if use_extracted_features:
            domains_array = np.array(domains or [Domain.real] * len(video_idx))
            real_indices = (domains_array == Domain.real).nonzero()[0]
            synth_indices = (domains_array == Domain.synthetic).nonzero()[0]

            if synth_indices.size > 0:
                synth_video_features_minibatch = self.encode_videos(videos_minibatch)  # fake list [mini_b, t, l, c]
                
                # sub_batch_size = videos_minibatch.size(0) // 2
                # synth_video_features_sub_batches = []
                # for start in range(0, videos_minibatch.size(0), sub_batch_size):
                #     end = min(start + sub_batch_size, videos_minibatch.size(0))
                #     synth_video_features_sub_batch = self.encode_videos(videos_minibatch[start:end])
                #     synth_video_features_sub_batches.append(synth_video_features_sub_batch)
                # synth_video_features_minibatch = torch.cat(synth_video_features_sub_batches, dim=0)
                
                b, t, l, c = real_video_features_minibatch.size()
                final_b = b + synth_video_features_minibatch.size(0)

                final_video_embeds = torch.zeros(final_b, t, l, c, device=real_video_features_minibatch.device, dtype=real_video_features_minibatch.dtype)
                final_video_embeds[real_indices] = real_video_features_minibatch
                final_video_embeds[synth_indices] = synth_video_features_minibatch
                video_features_minibatch = final_video_embeds  # fake list [mini_b, t, l, c]
            else:
                video_features_minibatch = real_video_features_minibatch  # fake list [mini_b, t, l, c]
        ...
        if shared_semantic_mask is None:
            shared_semantic_mask = torch.ones_like(input_ids, dtype=torch.bool)
        else:
            shared_semantic_mask = shared_semantic_mask.bool()
        ...
        shared_semantic_mask = [cur_sh_sem_mask[cur_attention_mask] for cur_sh_sem_mask, cur_attention_mask in zip(shared_semantic_mask, attention_mask)]

        new_input_embeds = []
        new_labels = []
        new_shared_semantic_mask = []
        cur_image_idx = 0
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                new_shared_semantic_mask.append(shared_semantic_mask[batch_idx])
                cur_image_idx += 1
                continue
            ...
            cur_labels_noim = []
            cur_shared_semantic_mask = shared_semantic_mask[batch_idx]
            cur_shared_semantic_mask_noim = []
            for i in range(len(image_token_indices) - 1):
                cur_input_ids_noim.append(cur_input_ids[image_token_indices[i]+1:image_token_indices[i+1]])
                cur_labels_noim.append(cur_labels[image_token_indices[i]+1:image_token_indices[i+1]])
                cur_shared_semantic_mask_noim.append(cur_shared_semantic_mask[image_token_indices[i]+1:image_token_indices[i+1]])
            split_sizes = [x.shape[0] for x in cur_labels_noim]
            cur_input_embeds = self.get_model().embed_tokens(torch.cat(cur_input_ids_noim))
            cur_input_embeds_no_im = torch.split(cur_input_embeds, split_sizes, dim=0)
            cur_new_input_embeds = []
            cur_new_labels = []
            cur_new_shared_semantic_mask = []

            for i in range(num_images + 1):
                cur_new_input_embeds.append(cur_input_embeds_no_im[i])
                cur_new_labels.append(cur_labels_noim[i])
                cur_new_shared_semantic_mask.append(cur_shared_semantic_mask_noim[i])
                if i < num_images:
                    # print(cur_image_idx)
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_token_index = image_token_indices[i+1]
                    cur_new_shared_semantic_mask.append(torch.full((cur_image_features.shape[0],), cur_shared_semantic_mask[cur_image_token_index], device=cur_shared_semantic_mask.device, dtype=cur_shared_semantic_mask.dtype))
                    cur_image_idx += 1
            ...
            cur_new_shared_semantic_mask = torch.cat(cur_new_shared_semantic_mask)

            new_input_embeds.append(cur_new_input_embeds)
            new_labels.append(cur_new_labels)
            new_shared_semantic_mask.append(cur_new_shared_semantic_mask)
        ...
        if tokenizer_model_max_length is not None:
            ...
            new_shared_semantic_mask = [x[:tokenizer_model_max_length] for x in new_shared_semantic_mask]
        ...
        shared_semantic_mask = torch.zeros((batch_size, max_len), dtype=new_shared_semantic_mask[0].dtype, device=new_shared_semantic_mask[0].device)
        position_ids = torch.zeros((batch_size, max_len), dtype=position_ids.dtype, device=position_ids.device)

        # for i, (cur_new_embed, cur_new_labels) in enumerate(zip(new_input_embeds, new_labels)):
        for i, (cur_new_embed, cur_new_labels, cur_new_shared_semantic_mask) in enumerate(zip(new_input_embeds, new_labels, new_shared_semantic_mask)):
            ...
            if getattr(self.config, 'tokenizer_padding_side', 'right') == "left":
                ...
                if cur_len > 0:
                    ...
                    shared_semantic_mask[i, -cur_len:] = cur_new_shared_semantic_mask
        ...
        # TODO: Check that the attention mask is correct
        if _attention_mask is None:
            attention_mask = None
        else:
            # attention_mask = attention_mask.to(dtype=_attention_mask.dtype)
            attention_mask = shared_semantic_mask.to(dtype=_attention_mask.dtype)
