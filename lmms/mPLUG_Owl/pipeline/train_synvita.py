...
from lmms.mPLUG_Owl.mplug_owl_video.modeling_mplug_owl_synvita import MplugOwlForConditionalGeneration
# from mplug_owl_video import MplugOwlForConditionalGeneration
from src.domain import Domain
from src.semantic_consistency_regularization import compute_semantic_consistency_regularization

from transformers.models.llama.tokenization_llama import LlamaTokenizer
from data_utils.__init__synvita import train_valid_test_datasets_provider
from utils import batchify, set_args, batchify_custom
...
parser.add_argument('--sem-cons-weight', type=float, default=1.0, help='Weight for semantic consistency regularization')  
parser.add_argument('--sem-cons-margin', type=float, default=0.2, help='Margin for semantic consistency regularization')  

class CustomTrainer(Trainer):
    def __init__(self, use_extracted_features, sem_cons_weight, sem_cons_margin, **kwargs):
        super().__init__(**kwargs)
        self.use_extracted_features = use_extracted_features
        self.sem_cons_weight = sem_cons_weight
        self.sem_cons_margin = sem_cons_margin
    ...
    def compute_loss(self, model, inputs, return_outputs = False):
        outputs = model(
            ...
            attention_mask=inputs['shared_semantic_mask'], 
            shared_semantic_indices=inputs['shared_semantic_indices'],
        )
        ...
        labels = outputs["labels"] if isinstance(outputs, dict) else outputs[2]

        loss_real, loss_synth = loss
        shared_semantic_indices = inputs['shared_semantic_indices'].cpu().numpy()
        dynamic_weights = torch.clamp(
            torch.tensor(inputs['alignment_diffs'], device=logits.device), min=0.0
        )

        if model.training:
            loss_real = loss_real.sum()
            self.log({"loss_real": loss_real.item()})

            if loss_synth is not None:
                synth_indices = (np.array(inputs['domains']) == Domain.synthetic).nonzero()[0]
                synth_indices = np.setdiff1d(synth_indices, shared_semantic_indices)
                synth_dynamic_weights = dynamic_weights[synth_indices]
                loss_synth = (loss_synth * synth_dynamic_weights).sum()
                self.log({"loss_synthetic": loss_synth.item()})
            else:
                loss_synth = torch.tensor(0.0, device=logits.device)

            loss_sem_cons = compute_semantic_consistency_regularization(
                logits, labels, shared_semantic_indices, inputs, dynamic_weights, self.sem_cons_margin, self.tokenizer
            )
            self.log({"loss_sem_cons": loss_sem_cons.item()})

            loss = loss_real + loss_synth + self.sem_cons_weight * loss_sem_cons
...
def main(): 
    ...
    tokenizer = LlamaTokenizer.from_pretrained(args.pretrained_ckpt, local_files_only=local_files_only)
    if args.use_lora:
    ...
    train_data, valid_data = train_valid_test_datasets_provider(
        config.data_dir, config.features_dir, config.data_files, config=config, 
    ...
    trainer = CustomTrainer(
        use_extracted_features=args.use_extracted_features,
        sem_cons_weight=args.sem_cons_weight,
        sem_cons_margin=args.sem_cons_margin,
        model=model,
