...
class MultiModalDataset(Dataset):
    ...
    def __init__(self, data_dir, features_dir, input_file, tokenizer, processor,
        ...
        self.dataset = pd.read_csv(input_file)
        self.dataset = self.dataset.replace({np.nan: None})
    ...
    def __getitem__(self, index):
        ...
        # ================================================
        optional_keys = ['alignment', 'alignment_diff', 'domain']
        optional_fields = {}
        for key in optional_keys:
            if key in data:
                if key == 'domain':
                    optional_fields[key] = Domain[data[key]]
                else:
                    optional_fields[key] = data[key] if data[key] is not None else 1.0
        # ================================================
