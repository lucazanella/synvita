...
from src.llama.modeling_llama import forward
...
class MplugOwlForConditionalGeneration(MplugOwlPreTrainedModel):
    ...
    def __init__(self, config: MplugOwlConfig):
        ...
        #     language_model = AutoModelForSeq2SeqLM.from_config(config.text_config)
        self.language_model = language_model
        self.language_model.forward = MethodType(forward, self.language_model)
    ...
    def forward(
        ...
        return_dict: Optional[bool] = None,
        use_extracted_features: Optional[bool] = None,
        ...
        if video_pixel_values is not None:
            if use_extracted_features:
                domains_array = np.array(domains)
                ...
                if synth_indices.size > 0:
                    real_video_embeds = video_pixel_values["video_embeds"]
                    synth_video_embeds = self.vision_model(video_pixel_values["video_pixel_values"], return_dict=True).last_hidden_state

                    b, t, n, d = real_video_embeds.size()
                    final_b = b + synth_video_embeds.size(0)

                    final_video_embeds = torch.zeros(final_b, t, n, d, device=real_video_embeds.device, dtype=real_video_embeds.dtype)

                    final_video_embeds[real_indices] = real_video_embeds
                    final_video_embeds[synth_indices] = synth_video_embeds

                    video_embeds = final_video_embeds
                else:
                    video_embeds = video_pixel_values
            else:
        ...
        num_images_per_sample = num_images.long()
        num_videos_per_sample = num_videos.long()
