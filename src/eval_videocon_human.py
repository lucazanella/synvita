...
def main(data_dir, input_csv_1, input_csv_2, output_file):
    ...
    df_hard = df[df["hard"]]
    auc_roc_score_hard = roc_auc_score(df_hard["label"], df_hard["entailment"])

    print(f"AUC-ROC: {100 * auc_roc_score}")
    print(f"AUC-ROC-Hard: {100 * auc_roc_score_hard}")
    with open(output_file, "w") as f:
        f.write(f"AUC-ROC: {(100 * auc_roc_score):.2f}\n")
        f.write(f"AUC-ROC-Hard: {(100 * auc_roc_score_hard):.2f}\n")
